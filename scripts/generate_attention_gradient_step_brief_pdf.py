...
def build_report() -> None:
    ...
    with PdfPages(REPORT_PATH) as pdf:
        ...
        ax.text(0.04, 0.885, "Монополия внимания: корректировка риск-градиента", fontsize=15.5, fontweight="bold", color=INK, va="top")
        ax.text(
            0.04,
            0.838,
        ...
        section(ax, 0.04, 0.775, "Математика")
        y = 0.728
        formulas = [
        ...
        for formula in formulas:
            ax.text(0.06, y, formula, fontsize=8.9, color=INK, va="top")
            y -= 0.056

        section(ax, 0.04, 0.535, "Смысл изменения")
        body = (
        ...
        wrapped_text(ax, 0.06, 0.492, body, width=78, fontsize=8.4, line_height=0.034)

        section(ax, 0.04, 0.300, "Калибровочные допущения")
        assumptions = (
        ...
        wrapped_text(ax, 0.06, 0.257, assumptions, width=78, fontsize=8.4, line_height=0.034)

        ax.add_patch(Rectangle((0.665, 0.115), 0.275, 0.210, transform=ax.transAxes, facecolor=LIGHT, edgecolor="#D1D5DB", linewidth=0.6))
        add_risk_image(fig, [0.675, 0.125, 0.255, 0.185])
        ax.text(0.665, 0.090, "Фрагмент обновленного риск-градиента", fontsize=7.5, color=MUTED, va="top")
